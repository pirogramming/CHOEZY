...
**수정 가능한 필드는 `purchase_status`, `satisfaction`, `purpose` 셋입니다.**
상품 정보와 나머지 스냅샷은 기록 당시 값이므로 바뀌지 않습니다.

| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `purchase_status` | choice | ✔ | `PURCHASED` / `DEFERRED` / `NOT_PURCHASED` |
| `satisfaction` | int | △ | `PURCHASED`일 때 **필수**, 1~5. 나머지는 **보내면 400** |
| `purpose` | choice | ✘ | `Consideration.Purpose` 값. **생략하면 그대로 둡니다** |

> **목적만 수정을 허용하는 이유**: 스냅샷이 막으려는 것은 원본 고민이나
> 회원 프로필의 변경이 과거 기록에 번지는 일입니다. 사용자가 상세 팝업에서
> 잘못 고른 자기 목적을 바로잡는 것은 그 대상이 아닙니다. 생성(§8.5)은
> 목적을 받지 않습니다 — 구매 결정 팝업에는 목적 입력이 없고 고민에서
> 그대로 복사합니다.
